<?php
    ...
    if ($result=mysqli_query($con,$sql))
    {
        ...
        $code .= tab() . '$sql = "INSERT INTO ' . getTableNameFromPrimaryKey($primaryKey) . '(' .$dbColumnNameWithoutPrefix[1];
        for($j=2;$j<sizeof($dbColumnName);$j++)
        {
            $code .= ", $dbColumnNameWithoutPrefix[$j]";
        }
        $code .= ') VALUES (';
        $code .= "'" . '$' . makeFirstLetterLowerCase($dbColumnName[1]) . "'";
        for($j=2;$j<sizeof($dbColumnName);$j++)
        {
            $code .= ", '" . '$' . makeFirstLetterLowerCase($dbColumnName[$j]) . "'";
        }
        $code .= ')";<br>';
        ...
        $code .= tab() . tab() . '//insert ผ่าน<br>';
        $code .= tab() . tab() . '$newID = mysqli_insert_id($con);<br>';
        $code .= '<br><br><br>';
        ...
        $code .= tab() . tab() . '//device ตัวเอง ลบแล้ว insert<br>';
        $code .= tab() . tab() . '//sync generated id back to app<br>';
        $code .= tab() . tab() . '//select row ที่แก้ไข ขึ้นมาเก็บไว้<br>';
        $code .= tab() . tab() . '$sql = "select ' . "$" . makeFirstLetterLowerCase($primaryKeyWithPrefix) . ' as ' . $primaryKey . ', 1 as ReplaceSelf, '. "'" . '$' . $varModifiedUser . "'" . ' as ModifiedUser";<br>';
        ...
        $code .= tab() . tab() . '//broadcast ไป device token ตัวเอง<br>';
        $code .= tab() . tab() . '$type = ' . "'" . getTableNameFromPrimaryKey($primaryKey) . "'" . ';<br>';
        $code .= tab() . tab() . '$action = ' ."'". 'd'. "'" . ';<br>';
        $code .= tab() . tab() . '$ret = doPushNotificationTaskToDevice($_POST["modifiedDeviceToken"],$selectedRow,$type,$action);<br>';
        $code .= tab() . tab() . 'if($ret != "")<br>';
        $code .= tab() . tab() . '{<br>';
        $code .= tab() . tab() . tab() . 'mysqli_rollback($con);<br>';
        $code .= tab() . tab() . tab() . 'putAlertToDevice();<br>';
        ...
        $code .= tab() . tab() . '//select row ที่แก้ไข ขึ้นมาเก็บไว้<br>';
        $code .= tab() . tab() . '$' . makeFirstLetterLowerCase($primaryKeyWithPrefix) . ' = $newID;<br>';
        $code .= tab() . tab() . '$sql = "select *, 1 IdInserted from ' . getTableNameFromPrimaryKey($primaryKey) . ' where ' . $primaryKey .' = ' . "'" . '$' . makeFirstLetterLowerCase($primaryKeyWithPrefix). "'". '";<br>';
        $code .= tab() . tab() . '$selectedRow = getSelectedRow($sql);<br>';
        $code .= '<br><br><br>';
        ...
        $code .= tab() . '//****device อื่น insert<br>';
